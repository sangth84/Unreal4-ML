...
sio = socketio.AsyncServer(logger=True, engineio_logger=True) #async_handlers=True
sio.eio.ping_timeout = 1200000
sio.eio.ping_interval = 500000
...
@sio.on('setObservationSpace', namespace="/")
async def set_observation_space(sid, value):
    global observation_space
    observation_space = int( value )
    
@sio.on('setAgentType', namespace="/")
async def set_Agent_Type(sid, value):
    global agentType
    agentType = value
...
async def start_script(sid, script_name):
	...
	valid, err = mlp.load(script_name, observation_space, agentType)
	if (err):
		print(err)
	print('loaded.')
