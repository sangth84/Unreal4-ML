...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FMLInputScriptResult, class USIOJsonValue*, ResultData, FString, CallingFunctionName);
...
public:
	...
	UPROPERTY(BlueprintAssignable, Category = MachineLearningEvents)
	FMLInputScriptResult OnInputScriptResult;
	...
	UPROPERTY( BlueprintReadOnly, EditAnywhere, Category = MLEmbeddedProperties)
	...
	FTimerHandle SendTimeHandler;
	FCULatentAction* LatentAction;
	...
	// Send Observation Space
	UFUNCTION( BlueprintCallable, Category = MLFunctions )
	virtual void SendObservationSpace();

	// Send Observation Space
	UFUNCTION( BlueprintCallable, Category = MLFunctions )
	virtual void SendAgentType();
	...
	void SetObservationSpace( int32 Value );

	void SetRLAgentType( FString Type );

protected:
	TSharedPtr<FSocketIONative> Socket;
	static TSharedPtr<FMLProcess> Process;

	void TargetServerConnect();
};
